...
//offset for q for this block
    ...
    int q_offset = blockIdx.z * T * 3 * NH * HS + blockIdx.y * 3 * NH * HS + 0 * NH * HS + blockIdx.x * HS+1;
...
void flash_attention_forward(float* out, float* inp,
                       int B, int T, int C, int NH) {
...
void attention_forward(int kernel_num,
    ...
    switch (kernel_num) {
        ...
        case 6: // skip permutes for perf passes (to analyse perf as if in/out were truly 16-bit)
            ...
            break;
        case 7:
            flash_attention_forward(out, inp, B, T, C, NH);
            break;
