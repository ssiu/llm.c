...
__global__ __launch_bounds__(256,2)
void fused_matmul_gelu_backward_kernel3(float* A, float* B, float* dinp, float* inp, int BT, int C, int OC){
    int thread_id = threadIdx.x;
    int block_idx = blockIdx.x;
    int block_idy = blockIdx.y;
...
//    fused_matmul_gelu_backward_kernel1<<<gridDim, blockDim>>>(dout, weight, dinp, pre_gelu_inp, B * T, C, OC);
    ...
    fused_matmul_gelu_backward_kernel3<<<gridDim, blockDim>>>(dout, weight, dinp, pre_gelu_inp, B * T, C, OC);
